...
namespace Nova.Server.Waypoints
{
    ...
    /// </summary>
    public class InvadeTaskWorker : InvadeTask, IWaypointTaskWorker
    {
        public InvadeTaskWorker()
        {
            
        }
        
        public InvadeTaskWorker(IWaypointTask invadeTask)
        {
            
        }
        
        public InvadeTaskWorker(XmlNode node) : base(node)
        {
        ...
        public bool isValid(Fleet fleet, Mappable target, ServerData serverState)
        {
            ...
            PlayerRelation relation = empire.EmpireReports[star.Owner].Relation;
            ...
            switch (relation)
            {
                ...
                case PlayerRelation.Neutral:
                    {
                        message.Text += star.Name + " but the " + empire.EmpireReports[star.Owner].RaceName + " are not our enemies. Order has been cancelled.";
                        return false;
                    }
        ...
        public bool Perform(Fleet fleet, Mappable target, ServerData serverState)
        {
            ...
            else if (survivorStrength < 0)
            {
                ...
                serverState.AllEmpires[star.Owner].OwnedStars.Remove(star);
                star.Owner = fleet.Owner;
                serverState.AllEmpires[fleet.Owner].OwnedStars.Add(star);
                serverState.AllEmpires[fleet.Owner].StarReports[star.Key].Update(star, ScanLevel.Owned, serverState.TurnYear);
